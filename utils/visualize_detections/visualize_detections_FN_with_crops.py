...
def load_config(config_file=None):
    ...
    default_config = {
        ...
        "output_dir": r"D:\PLACE - Zotac\BGD\Testing\visualize_FN",
        "crops_dir": r"D:\PLACE - Zotac\BGD\Testing\crops_FN",
        "save_crops": True,
        "line_thickness": 2,
        "font_scale": 0.6,
        "jpg_quality": 85
...
def get_color_for_class(class_name):
    """Get BGR color based on class name."""
    class_name_lower = class_name.lower()
    
    if 'face' in class_name_lower:
        return (0, 255, 0)  # Green for faces
    elif 'license' in class_name_lower or 'plate' in class_name_lower:
        return (0, 0, 255)  # Red for license plates
    else:
        return (255, 255, 0)  # Cyan for other
...
def process_images_recursive(
    ...
    """
    Process all images with Stage 2 detections and create visualizations + crops.
    
    Args:
        input_dir: Directory containing images and *_FN.txt files
    ...
    logger.info(f"Starting Stage 2 visualization and cropping (faces & license plates)...")
    ...
    for idx, image_path in enumerate(image_files, 1):
        try:
            ...
            # Look for Stage 2 detection file
            detection_text_path = image_path.parent / f"{image_path.stem}_FN.txt"
            
            if not detection_text_path.exists():
                logger.debug(f"[{idx}/{len(image_files)}] No FN detection file for: {relative_path}")
                skipped_count += 1
                continue
            ...
            output_viz_path = output_subdir / f"{image_path.stem}_visualization_FN.jpg"
    ...
    logger.info("Color coding: GREEN=faces, RED=license plates")
    logger.info("=" * 70)

def main():
    """Main function with CLI argument support."""
    parser = argparse.ArgumentParser(
        description='Visualize Stage 2 detections from _FN.txt files with optional crop output',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Visualize only (no crops)
  python visualize_detections_FN_with_crops.py --no-crops
  
  # Visualize and save crops
  python visualize_detections_FN_with_crops.py --config config_viz_FN.json
  
  # Custom crop directory
  python visualize_detections_FN_with_crops.py --crops-dir /path/to/crops
  
  # Thicker lines and high quality
  python visualize_detections_FN_with_crops.py --line-thickness 3 --jpg-quality 95
        """
    )
    
    parser.add_argument('--config', type=str, help='Path to JSON configuration file')
    parser.add_argument('--input-dir', type=str, help='Input directory with images and *_FN.txt files')
    ...
    logger.info("Starting Stage 2 visualization with configuration:")
    for key, value in config.items():
        logger.info(f"  {key}: {value}")
